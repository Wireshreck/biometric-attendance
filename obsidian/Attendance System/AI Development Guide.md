...
The project is a local-first science-fair attendance demonstrator planned around an ESP32 + AS608 sensor, local FastAPI service, SQLite, and a browser dashboard. The SQLite schema foundation exists; the attendance product, API, firmware, dashboard, and physical system are not implemented. Use synthetic identities and consenting adult testers for any future demo. The optional local AI reporting layer is deferred and must remain read-only and unnecessary for core operation.
...
| `backend/` | SQLite schema v1 migration and tests; Python dependency/environment setup; no application/routes |
...
| Backend | `backend/README.md`, `backend/pyproject.toml`, and actual sources under `backend/app/` when added |
| Database | `docs/database-plan.md` and checked-in migrations under `backend/migrations/` when implemented |
| API | `docs/api-plan.md` and actual route code when implemented |
...
- **DECIDED** and **PROVISIONAL** may appear in ADR decision fields only: accepted project choices and choices awaiting hardware/evidence.
...
10. Update this project's handoff/status only when the next agent would otherwise lack current context.
...
`test_env.py` is an environment/SQLite smoke check, not an API test. The current pytest suite verifies schema migration/constraints only. PlatformIO may need network access to fetch its pinned toolchain; flashing and sensor checks require hardware. Use the exact commands in each subsystem README and test plan.
